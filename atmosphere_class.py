...
class AtmosphereDef:
    ...
    def atm_mass(self, planet):
        ...
        """
        # p is converted from bar to Pa (kg m-1 s-2, *1e5) to g m-1 s-2 (*1e3)
        return ((self.p_surf * 1e8) * planet.surface_area) / planet.g
    ...
    def initial_comp(self, planet, mantle, fastchem):
        ...
        if names:
            ...
            answer = msgs.query_yes_no(
                f"The species {names} are listed in the initial atmospheric composition but are not being tracked to determine if atmospheric steady state has been reached.\nDo you wish to add these species to the tracked list?"
            )
        ...
        if fastchem != True and (self.t_surf != mantle.t):
            print(
                f"Without FastChem enabled, the atmospheric composition will be for a gas at {mantle.t} K rather than the surface temperature {self.t_surf}.\n"
            )

            answer = msgs.query_yes_no(
                "Would you like to continue by turning on FastChem?", default="yes"
            )
            ...
            if answer == True:
                ...
                answer = msgs.query_yes_no(
                    f"Do you wish to continue by setting the surface temperature to {mantle.t} K?",
                    default=None,
                )
                if answer == True:
                    ...
                    msgs.exiting(
                        "EVolve cannot generate an atmosphere cooler than magmatic temperature without FastChem."
                    )
    ...
    def unitconvert_atomicmass2fc(self, masses=None):
        ...
        if masses == None:
            # the total mass of each element in the atmosphere
            mols = self.total_atomic_mass
        else:
            mols = masses

        # mass fractions of each element in the atmosphere
        atm_norm = cnvs.wt2mol(cnvs.mass2wt_frac(mols))
    ...
    def fc_filecreate(self, t_step):
        ...
        if file_exists:
            with open("outputs/fc_input.csv", "a") as f_inp:
                f_inp.write(
                    f"{t_step} \t {self.t_surf:.3f} \t {self.p_surf:.16f} \t {self.fc_cheminput['h']} \t {self.fc_cheminput['c']} \t {self.fc_cheminput['o']} \t {self.fc_cheminput['s']} \t {self.fc_cheminput['n']}\n"
                )
    ...
    def run_fastchem(self, planet, t_step):
        ...
        fastchem = pyfastchem.FastChem(
            "fastchem/input/evolve_abundances.dat", "fastchem/input/logK/logK.dat", 1
        )
        ...
        if result != 0:
            msgs.exiting(pyfastchem.FASTCHEM_MSG[result])
    ...
    def fc_saveoutput(self, planet, tstep, output_data, fastchem):
        ...
        mmw, tracked_sp = saveChemistryOutput(
            filename,
            self.t_surf,
            self.p_surf,
            output_data,
            fastchem,
            tstep,
            tracked_species=self.tracked_species,
        )
    ...
    def update_store(self, volc, melt_mass):
        ...
        for key in self.total_atomic_mass.keys():
            # atomicM_gas is still the weight fractions of the system, just only the ones in the gas phase.
            wt = volc.atomicM_gas[key.lower()] * 1e-6 * melt_mass

            # mass of each element in the atmosphere (circumventing fact we only track a few species from fastchem)
            self.total_atomic_mass[key] += wt
    ...
    def mix_atmosphere(self, p_new, p_volc):
        ...
        # stores the new atmospheric composition as mol fractions ready to save in csv file.
        new_comp = {}

        for sp in self.composition:
            if sp in self.atmos_input:
                pi_volc = (
                    self.atmos_input[sp] * p_volc
                )  # atmos_input is the input from evo/fc as mol fractions
    ...
    def scale_height(self, planet):
        ...
        # pressure scale height in m, mmw converted to kg/mol
        H = (cnst.R * self.t_surf) / ((self.mmw / 1e3) * planet.g)
    ...
    def check_evolution(self, tol=1e-3):
        ...
        for ele in self.composition.keys():
            if (
                abs(self.composition[ele][0] - self.composition[ele][1])
                / self.composition[ele][0]
                > tol
            ):
