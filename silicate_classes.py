...
class PlanetDef:
    ...
    def setup_conservation(self, mantle, atm):
        ...
        mantle_h, mantle_c, mantle_s, mantle_n = mantle.calc_atomic_masses(
            mantle.mantle_volatiles
        )  # atomic fractions in ppm
        ...
        atm_comp_mass = {
            v: atm_wt_frac[v] * atm_mass for v in atm.composition.keys()
        }  # mass of each molecule in atmosphere (weight fraction * atmospheric mass)

        H = 2 * cnst.m["h"] * (
            (atm_comp_mass["H2O"] / cnst.m["h2o"])
            + (atm_comp_mass["H2"] / cnst.m["h2"])
            + (atm_comp_mass["H2S"] / cnst.m["h2s"])
            + 2 * (atm_comp_mass["CH4"] / cnst.m["ch4"])
            + (atm_comp_mass["C2H2"] / cnst.m["c2h2"])
        ) + cnst.m["h"] * (
            (atm_comp_mass["HCN"] / cnst.m["hcn"])
            + 3 * (atm_comp_mass["NH3"] / cnst.m["nh3"])
        )

        C = cnst.m["c"] * (
            (atm_comp_mass["CO2"] / cnst.m["co2"])
            + (atm_comp_mass["CO"] / cnst.m["co"])
            + (atm_comp_mass["CH4"] / cnst.m["ch4"])
            + (atm_comp_mass["HCN"] / cnst.m["hcn"])
            + 2 * (atm_comp_mass["C2H2"] / cnst.m["c2h2"])
        )

        S = cnst.m["s"] * (
            (atm_comp_mass["SO2"] / cnst.m["so2"])
            + 2 * (atm_comp_mass["S2"] / cnst.m["s2"])
            + (atm_comp_mass["H2S"] / cnst.m["h2s"])
        )

        N = cnst.m["n"] * (
            2 * (atm_comp_mass["N2"] / cnst.m["n2"])
            + (atm_comp_mass["HCN"] / cnst.m["hcn"])
            + (atm_comp_mass["NH3"] / cnst.m["nh3"])
        )

        O = cnst.m["o"] * (
            (atm_comp_mass["H2O"] / cnst.m["h2o"])
            + 2 * (atm_comp_mass["O2"] / cnst.m["o2"])
            + 2 * (atm_comp_mass["CO2"] / cnst.m["co2"])
            + (atm_comp_mass["CO"] / cnst.m["co"])
            + 2 * (atm_comp_mass["SO2"] / cnst.m["so2"])
        )
    ...
    def check_conservation(self, mantle, atm, nocrust=False):
        ...
        for v in vols:
            ...
            mantle_vols[v] = (mantle_vols[v] * mantle.mass) / cnst.m[
                v.lower()
            ]  # number of mols of each species (mass/molar_mass)

            if nocrust == False:
                crust_vols_mass[v] = mantle.crust[v] / 1e6
                crust_vols_mass[v] = (
                    crust_vols_mass[v] * mantle.crust["mass"]
                ) / cnst.m[v.lower()]
            else:
                crust_vols_mass[v] = 0

        H = (
            2 * cnst.m["h"] * (mantle_vols["H2O"] + crust_vols_mass["H2O"])
            + atm.total_atomic_mass["H"]
        )

        C = (
            cnst.m["c"] * (mantle_vols["CO2"] + crust_vols_mass["CO2"])
            + atm.total_atomic_mass["C"]
        )

        S = (
            cnst.m["s"] * (mantle_vols["S"] + crust_vols_mass["S"])
            + atm.total_atomic_mass["S"]
        )

        N = (
            cnst.m["n"] * (mantle_vols["N"] + crust_vols_mass["N"])
            + atm.total_atomic_mass["N"]
        )

        if (
            abs(self.total_volatiles["h"] - H) / H > 1e-10
            or abs(self.total_volatiles["c"] - C) / C > 1e-10
            or abs(self.total_volatiles["s"] - S) / S > 1e-10
            or abs(self.total_volatiles["n"] - N) / N > 1e-10
        ):
...
class MantleDef:
    ...
    def __init__(self):
        ...
        self.crust = (
            {}
        )  # store of volatiles that didn't degass according to IE ratio PL: return these to mantle or store as inaccessible?
    ...
    def init_store(self):
        ...
        data = {
            "t_step": 0,
            "age_yr": 0,
            "mantle_mass": "{:3.4e}".format(self.mass),
            self.fo2_buffer: self.fo2,
        }
    ...
    def batch_melt(self, F, vol):
        ...
        if vol.lower() != "n":
            return self.mantle_volatiles[vol] / (
                cnst.D[vol.lower()] + F * (1 - cnst.D[vol.lower()])
            )
        else:  # N
            return self.mantle_volatiles[vol] / (
                cnst.D["n_fefeo"] + F * (1 - cnst.D["n_fefeo"])
            )  # PL: Only using the reduced form at the moment, either turn into linear equation or split according to which is closer.
    ...
    def generate_melt(self, planet, atm, tstep, nocrust=False):
        ...
        melt_density = (
            density(cnvs.wt2mol(self.composition), self.t, atm.p_surf) * 1e3
        )  # g/m3

        # mass of melt erupted to the surface in one year
        melt_mass = melt_density * planet.melt_flux

        # the amount of melt erupted to the surface (volcanic flux), g, in the whole time period.
        melt_mass *= planet.timestep

        # total mass of melt produced in the mantle
        total_melt_mass = melt_mass * 1 / self.ie_ratio

        # amount of that melt which will get stored in the crust undegassed
        intrusive_mass = total_melt_mass - melt_mass

        melt_volatile = {}

        for species in self.mantle_volatiles.keys():
            # CONCENTRATION (ppm) of volatile in the melt.
            melt_volatile[species] = self.batch_melt(self.F, species)
            ...
            if nocrust == True:
                ...
                self.mantle_volatiles[species] = self.mix_reserviors(
                    self.mass,
                    self.mantle_volatiles[species],
                    melt_mass,
                    melt_volatile[species],
                    removing=True,
                )
                ...
                if species in self.crust:
                    self.crust[species] = self.mix_reserviors(
                        self.crust["mass"],
                        self.crust[species],
                        intrusive_mass,
                        melt_volatile[species],
                    ...
                    # when there is no pre-existing crust (in the model), the CONCENTRATION of volatile in crust and melt will be equal.
                    self.crust[species] = melt_volatile[species]

        if nocrust == True:
            # only remove the stuff that's going into the volcano.
            self.mass -= melt_mass
    ...
    def update_crust(
        self, planet, volc, atm, concs, WgT, melt_mass, melt_v, tstep, nocrust=False
    ):
        """
        ...
        # volatiles in the degassed melt converted into atomic weight fractions (ppm)
        h, c, s, n = (
            volc.atomicM_melt["h"],
            volc.atomicM_melt["c"],
            volc.atomicM_melt["s"],
        ...
        if nocrust == True:
            ...
            crust_density = (
                density(cnvs.wt2mol(self.composition), atm.t_surf, atm.p_surf) * 1e3
            )  # approx crustal density at surface PT conditions

            crust_vol = self.crust["mass"] / crust_density

            thickness = (
                crust_vol / planet.surface_area
            )  # PL: this assumes a flat surface not a sphere...
    ...
    def calc_atomic_masses(self, melt_vols):
        ...
        s = melt_vols[
            "S"
        ]  # These are already == to atomic mass fractions as they dissolve as ions with no O.
        n = melt_vols["N"]
        ...
        h = (
            2
            * cnst.m["h"]
            * ((h2o / cnst.m["h2o"]) + (h2 / cnst.m["h2"]) + 2 * (ch4 / cnst.m["ch4"]))
        )
        c = cnst.m["c"] * (
            (co2 / cnst.m["co2"]) + (co / cnst.m["co"]) + (ch4 / cnst.m["ch4"])
        )
    ...
    def set_evo_env(self, volc, atm, set_atomic_weights=False):
        ...
        yaml = (
            ruamel.yaml.YAML()
        )  # ensures order of keys, comments and whitespace of file are maintained.
...
class VolcanoDef:
    ...
    def __init__(self, atm):
        ...
        # SYSTEM MASS FRACTIONS (PPM) of volatiles only in the GAS phase.
        self.atomicM_gas = {
            "c": 0,
            "h": 0,
            "n": 0,
        ...
        # SYSTEM MASS FRACTIONS (PPM) of volatiles only in the MELT phase.
        self.atomicM_melt = {
            "c": 0,
            "h": 0,
            "n": 0,
    ...
    def get_atomicM_gas(self, WgT):
        ...
        self.atomicM_gas["o"] = (
            ...
            * (
                (WgT / sum(mjMj))
                * (mf["H2O"] + 2 * mf["O2"] + 2 * mf["CO2"] + mf["CO"] + 2 * mf["SO2"])
            )
        )
        ...
        self.atomicM_gas["h"] = (
            ...
            * cnst.m["h"]
            * 1e6
            * ((WgT / sum(mjMj)) * (mf["H2O"] + mf["H2"] + 2 * mf["CH4"] + mf["H2S"]))
        )

        self.atomicM_gas["c"] = (
            cnst.m["c"] * 1e6 * ((WgT / sum(mjMj)) * (mf["CO"] + mf["CO2"] + mf["CH4"]))
        )

        self.atomicM_gas["s"] = (
            cnst.m["s"]
            * 1e6
            * ((WgT / sum(mjMj)) * (2 * mf["S2"] + mf["H2S"] + mf["SO2"]))
        )
    ...
    def run_evo(self, tstep):
        ...
        if self.out != "None":
            out = self.out
            try:
                evo_df = evo.main(chem, env, out)
            except Exception as e:
                msgs.exiting(f"Exiting: EVo failed to run. :(\nError reported: {e}")

        else:
            try:
                evo_df = evo.main(chem, env, None)
            except Exception as e:
                msgs.exiting(f"Exiting: EVo failed to run. :(\nError reported: {e}")

        # Check the planet surface pressure and final EVo run pressure match

        if not np.isclose(evo_df["P"].iloc[-1], self.atm.p_surf, atol=1e-3):
            msgs.exiting(
                f"Error: Final pressure of EVo run ({evo_df['P'].iloc[-1]} bar) does "
                f"not match required surface pressure ({self.atm.p_surf} bar)."
            )

        self.get_molmelt_fracs(evo_df)
        self.mmw = evo_df["mol_mass"].iloc[-1]

        # convert to weight fraction from percentage
        self.WgT = evo_df["Gas_wt"].iloc[-1] / 100

        self.init_graph_sat = True if evo_df["graph_melt"].iloc[0] != 0.0 else False

        # save the atomic weight fractions (ppm) in the gas phase only.
        self.get_atomicM_gas(self.WgT)

        for k in self.atomicM_gas.keys():
            self.atomicM_melt[k] = (
                self.atomicM_sys[k] - self.atomicM_gas[k]
            )  # rest of the system volatiles must be in the melt
        ...
        if file_exists:
            ...
            evo_df.to_csv(
                "outputs/volc_out.csv", mode="a", header=False, sep="\t", index=False
            )
        else:
            evo_df = evo_df.iloc[[-1]]
            evo_df.insert(loc=0, column="t_step", value=tstep)
